...
def render_new_onboarding_page(db: Session):
    ...
    if st.session_state["form_step"] == 1:
        ...
        with st.form("new_associate_form", clear_on_submit=False):
            ...
            with j3:
                grade = st.selectbox("Grade", options=GRADES, index=0)
            ...
            work_mode = st.radio("Work Mode *", options=WORK_MODES, horizontal=True, index=0)
            ...
            with col_b1:
                btn_draft = st.form_submit_button("Save Draft", use_container_width=True)
            with col_b2:
                btn_cancel = st.form_submit_button("Cancel", use_container_width=True)
            with col_b3:
                btn_continue = st.form_submit_button("Continue to Review", type="primary", use_container_width=True)
