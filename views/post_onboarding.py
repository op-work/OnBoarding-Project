...
def render_post_onboarding_page(db: Session):
    ...
    # Post-Onboarding Stage Progress Bar (Percentage on the Left)
    p_col1, p_col2 = st.columns([1.2, 4.8])
    with p_col1:
        st.markdown(f"<div style='font-size: 16px; font-weight: 700; color: #1E40AF; padding-top: 2px;'>{stage_progress['progress_pct']}%</div><div style='font-size: 12px; color: #64748B;'>({stage_progress['completed']}/4 Verified)</div>", unsafe_allow_html=True)
    with p_col2:
    ...
    if st.button("Back to Associate Workspace", key="btn_back_onb_post", type="primary", use_container_width=True):
